...
public class SymmetricEncryptionController {
	// C�digo interno que representa el algoritmo.
		...
		try {
			...
			if (inputStrSinDes.length() != 0 && inputStrModOpe != null && inputStrKeyCif.length() != 0) {
				...
				Alert alert = new Alert(AlertType.ERROR,
						UtilMessage.getMensaje("edu.udb.cri.system.alert.information.encryption"));
				alert.showAndWait();
			}
		} catch (Exception ex) {
	...
	void DecodeDes(ActionEvent event) {
		...
		try {
			...
			if (inputStrCifDes.length() != 0 && inputStrModOpe != null && inputKeyDescifraDes.length() != 0) {
				...
				Alert alert = new Alert(AlertType.ERROR,
						UtilMessage.getMensaje("edu.udb.cri.system.alert.information.decode"));
				alert.showAndWait();
			}
		} catch (Exception ex) {
			Alert alert = new Alert(AlertType.ERROR, ex.getMessage());
